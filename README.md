...
V1.5.4 起，POS 設定頁可按「檢查更新」比對 `version.json`。此功能只檢查版本，不會在 POS 內直接安裝；正式更新仍由 GitHub `main` → Zeabur 部署。

## 安全機制

V1.5.4 起：PIN 錯誤 5 次鎖定 15 分鐘；新設/修改 PIN 至少 6 碼；Session 12 小時；CSRF、Secure Cookie 與安全標頭；下載完整資料庫及刪除使用者需二次 PIN 驗證。
